...
### AnirudhGPT Configuration
Before running AnirudhGPT with Docker, you need to configure some settings:
...
### Running AnirudhGPT
Once you have completed all the setup, you can start the AnirudhGPT application by running the command `docker-compose up` in your terminal. After running this command, you can access the AnirudhGPT application at `http://localhost:3080`.
...
## Updating AnirudhGPT
To update AnirudhGPT. enter these commands one after the other from the root dir:
- git pull
...
```yaml
    ...
    environment:
      - HOST=0.0.0.0
      - MONGO_URI=mongodb://mongodb:27017/AnirudhGPT
# ...
...
```yaml
      args:
        APP_TITLE: AnirudhGPT # default, change to your desired app name
```
...
```yaml
     # image: ghcr.io/techwithanirudh/chatgpt-clone:latest # Uncomment this & comment above to build from docker hub image
```
...
### **[AnirudhGPT on Docker Hub](https://hub.docker.com/r/chatgptclone/app/tags)**
